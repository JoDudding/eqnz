---
title: "#eqnz - latest 100 MMI 3+ earthquakes"
...
# set location to NZ
x <- Sys.setlocale("LC_ALL", "en_NZ.utf8")
...
#| label: latest-mag
#| component: valuebox
#| title: Magnitude of last earthquake
list(
  icon = "activity",
  color = latest_colour,
  value = latest_mag
...
eqnz |> 
  ...
  transmute(
    #colour,
    ID = glue('[{publicID}]({link})'),
    Summary = glue('**MMI {mmi} - {intensity}**\n\n{
      since} - {format(time, "%a, %d %B %Y %I:%M %p")}\n\nMagnitude {
      comma(magnitude, 0.1)} with depth of {comma(depth, 1)} km\n\n{locality}')
  ) |> 
  gt() |> 
  fmt_markdown() |> 
  cols_align('left') |> 
...
eqnz |> 
  ...
  labs(
    x = NULL, y = NULL,
    title = 'Quake trend of magnitude'
  )
```
...
eqnz |> 
  arrange(time) |> 
  ggplot(aes(colour = colour, size = fct_rev(intensity))) +
  geom_sf(aes()) +
  scale_colour_identity() +
  labs(
    x = NULL, y = NULL,
...
Last updated: `r format(time_now, "%a, %d %B %Y %I:%M %p")`
